#!/bin/bash
# Quick Start Script for Steel Defect Detection Project
# This script sets up the environment and runs a complete demo pipeline
...
# Step 3: Download dataset instructions
...
if [ ! -d "data/raw/IMAGES" ]; then
    ...
    echo ""
    read -p "Press Enter when dataset is ready, or Ctrl+C to exit..."
fi
echo ""
...
# Step 6: Quick training demo (1 epoch)
echo "Step 6: Running quick training demo (1 epoch)..."
read -p "Run training demo? This will take a few minutes. (y/n) " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
...
# Step 7: Instructions for full training
echo "=========================================="
...
echo "   python src/train_yolo.py --data data/processed/yolo/data.yaml --epochs 50 --device cuda:0"
echo ""
echo "2. Full DETR training (100 epochs, GPU recommended):"
echo "   python src/train_detr.py --data data/processed/coco --epochs 100 --device cuda:0"
echo ""
